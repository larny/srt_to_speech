...
async def _main(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for i, j in enumerate(srt_data):
            ...
            start = pysrttime_to_seconds(j.start)-0.31
            end = pysrttime_to_seconds(j.end)+0.31

            input_files_start_end[fname] = (start, end)

            duration = pysrttime_to_seconds(j.duration)+0.8
...
def main():
    dep_check()
    srt_all = glob.glob(os.path.join("./", "*.srt"))
    mp3_name = srt_all[0][:-4] + ".mp3"
    parser = argparse.ArgumentParser(description="Converts srt to mp3 using edge-tts")
    parser.add_argument("--srt_file", help="srt file to convert", default=srt_all[0])
    parser.add_argument("--out_file", help="output file", default=mp3_name)
    # Aria
    parser.add_argument("--voice", help="voice name", default="en-US-ChristopherNeural")
